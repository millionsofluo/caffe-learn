...
//  应用更新，由solver.cpp 的 void Solver<Dtype>::Step(int iters)  调用
//  这一个函数的具体计算框架在笔记本上有，6.7的
template <typename Dtype>
void SGDSolver<Dtype>::ApplyUpdate() {
  ...
  ClipGradients();
    //  对训练网络中的每个权值blob都做归一化，正则化，计算增量这三步骤
      // 对所有可更新的网络参数进行操作
...
//  归一化函数，沿iter维度归一化，就是diff全部除以iter_size
template <typename Dtype>
...
void SGDSolver<Dtype>::Regularize(int param_id) 
  ...
  //  得到solver.prototxt 中的weight_decay
  Dtype weight_decay = this->param_.weight_decay();
   //  得到solver.prototxt 中的regularization_type
...
void SGDSolver<Dtype>::SnapshotSolverStateToBinaryProto(
  ...
  state.set_current_step(this->current_step_);  //  记录当前前进步数值
  state.clear_history();  //清空容器准备接纳新内容
  for (int i = 0; i < history_.size(); ++i) {
    // Add history  记录权值历史信息
  ...
  WriteProtoToBinaryFile(state, snapshot_filename.c_str());  //  将solverstate写入二进制文件
}
